...
// Helper function to ensure type safety with eq comparisons
const eqUser = (userId: number) => eq(sessions.userId, userId);
...
export function registerRoutes(app: Express): Server {
  ...
  app.get("/api/messages/:subject", requireAuth, async (req, res) => {
    ...
    const sessionMessages = await db.query.messages.findMany({
      where: and(
        ...
        eq(messages.userId, req.user!.dbId)
      ),
  ...
  // Update other routes to include user context
  app.post("/api/messages", requireAuth, async (req, res) => {
    const { subject, message } = req.body;
    const response = await generateExplanation(subject);

    await db.insert(messages).values([
      { role: "user", content: message, subject, userId: req.user!.dbId },
      { role: "assistant", content: response.explanation, subject, userId: req.user!.dbId }
    ]);
